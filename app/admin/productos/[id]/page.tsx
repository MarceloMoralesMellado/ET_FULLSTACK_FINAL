  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Check for schema warnings from backend
        const data = await response.json()
        if (data._warning) {
          toast({
            title: "Advertencia: Campos Faltantes en BD",
            description: `${data._warning.message} ${data._warning.fields.join(", ")}. Ve al Admin de PocketBase y créalos.`,
            variant: "destructive",
            duration: 10000,
          })
        }
