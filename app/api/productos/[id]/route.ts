...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // DEBUG: Log retrieved data to verify persistence
    console.log(`[DEBUG GET] Product ${id}:`, {
      oferta: producto?.enOferta,
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (body.destacado !== undefined) {
      datosActualizar.destacado = body.destacado
      datosActualizar.isDestacado = body.destacado
    }
    if (body.enOferta !== undefined) {
      datosActualizar.oferta = body.enOferta
      datosActualizar.enOferta = body.enOferta
    }
    if (body.precioOferta !== undefined) {
      const val = Number(body.precioOferta)
      datosActualizar.precio_oferta = val
      datosActualizar.precioOferta = val
    }
    ...
    try {
      ...
      const productoActualizado = {
        ...
        destacado: record.destacado || record.isDestacado || false,
        enOferta: record.oferta || record.enOferta || false,
        activo: record.disponible,
      }

      // DIAGNÓSTICO DE SCHEMA: Verificar si los campos realmente existen en la respuesta
      const missingFields = []
      if (datosActualizar.oferta !== undefined && record.oferta === undefined && record.enOferta === undefined) {
        missingFields.push("oferta (o enOferta)")
      }
      if (datosActualizar.precio_oferta !== undefined && record.precio_oferta === undefined && record.precioOferta === undefined) {
        missingFields.push("precio_oferta (o precioOferta)")
      }
      if (datosActualizar.destacado !== undefined && record.destacado === undefined && record.isDestacado === undefined) {
        missingFields.push("destacado (o isDestacado)")
      }

      if (missingFields.length > 0) {
        console.warn("⚠️ ALERTA DE SCHEMA: Los siguientes campos fueron enviados pero PocketBase no los devolvió (¿Falta crearlos en el Admin?):", missingFields)
        return NextResponse.json({
          ...productoActualizado,
          _warning: {
            message: "Los datos se enviaron, pero la base de datos no los guardó. Campos faltantes:",
      ...
      if (datosActualizar.categoria && status === 400) {
        ...
        try {
          ...
          // DIAGNÓSTICO DE SCHEMA EN RETRY
          // Verificar si los campos realmente existen en la respuesta del retry
          const missingFieldsRetry = []
          if (datosActualizar.oferta !== undefined && recordRetry.oferta === undefined && recordRetry.enOferta === undefined) {
            missingFieldsRetry.push("oferta (o enOferta)")
          }
          if (datosActualizar.precio_oferta !== undefined && recordRetry.precio_oferta === undefined && recordRetry.precioOferta === undefined) {
            missingFieldsRetry.push("precio_oferta (o precioOferta)")
          }
          if (datosActualizar.destacado !== undefined && recordRetry.destacado === undefined && recordRetry.isDestacado === undefined) {
            missingFieldsRetry.push("destacado (o isDestacado)")
          }

          if (missingFieldsRetry.length > 0) {
            console.warn("⚠️ ALERTA DE SCHEMA (RETRY): Campos faltantes:", missingFieldsRetry)
            return NextResponse.json({
              ...productoRetry,
              _warning: {
                message: "Los datos se enviaron (en reintento), pero la base de datos no los guardó. Campos faltantes:",
                fields: missingFieldsRetry
              }
            })
          }
