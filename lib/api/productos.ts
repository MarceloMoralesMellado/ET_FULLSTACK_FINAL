...
function convertirProducto(record: ProductoRecord): Producto {
  return {
    ...
    precioOferta: record.precio_oferta || record.precioOferta || 0,
...
export async function obtenerProductos(pbClient?: any): Promise<Producto[]> {
  try {
    ...
    const records = (await client.collection("productos").getFullList({
      sort: "-created",
    })) as unknown as ProductoRecord[]
...
export async function obtenerProductoPorId(id: string, pbClient?: any): Promise<Producto | null> {
  try {
    const client = pbClient || pb
    const record = (await client.collection("productos").getOne(id)) as unknown as ProductoRecord
